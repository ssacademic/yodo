...
```
Enter          new line            Tab / ⇧Tab     tuck under / pull out
↑ ↓            walk the outline    ⌥⇧↑ / ⌥⇧↓      move a line among its siblings
⌫ on empty     remove the line     ⌘Z / Ctrl-Z    undo the last removal
```

## Accessibility

Text contrast, tap areas and layout are checked by a script rather than by eye
(`audit.js` in the source repo). Current state: every text colour clears WCAG 1.4.3,
every control clears the WCAG 2.5.8 minimum of 24px, and 30 of 34 reach Apple's 44px.
The four that don't are limited by row height and sit around 40px — reported rather than
papered over.

Not yet done: a screen-reader pass on a real phone. Every control carries an `aria-label`,
but that is not the same as having tested it.
